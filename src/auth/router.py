...
from src.user.schemas import NewUser, LoginUser
...
def register(request: NewUser, db: Session = Depends(get_db)):
    ...
    user = Users(name=request.name, email=request.email, password=hashed_password)
    db.add(user)
    db.commit()
    db.refresh(user)
...
@router.post("/token")
def login_for_access_token(request: LoginUser, db: Session = Depends(get_db)):
    user = db.query(Users).filter(Users.email == request.email).one()
    if not user:
        return False
